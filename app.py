...
app.layout = html.Div(children=[
    ...
    html.Div([
        ...
        html.Div([
            ...
            dcc.Dropdown(id='input-type', options=[  # REVIEW2: Dropdown options
...
@app.callback(
    [
        Output(component_id=f'plot{i}', component_property='children') for i in range(1, 6)
    ],
    [
    ...
    [
        State(f"plot{i}", 'children') for i in range(1, 6)
    ])
...
def get_graph(chart, year, children1, children2, c3, c4, c5):
    ...
    if chart == 'OPT1':
        ...
        line_fig = px.line(line_data, x='Month', y='Reporting_Airline',
                           color='Reporting_Airline', title='Average flight time by reporting airline')
        ...
        tree_fig = px.treemap(tree_data, path=['Reporting_Airline', 'DestState'],
                              ...
                              title='Number of flights to destination state by reporting airline')
...
if __name__ == '__main__':
    ...
    app.run_server(mode="external", host="0.0.0.0",
                   dev_tools_ui=False , dev_tools_props_check=False, port = 8080)
